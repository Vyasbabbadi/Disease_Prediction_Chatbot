...
# Import data (only for fitting vectorizer)
...
# Model preparation
model = RNN_model()
model.load_state_dict(torch.load('pretrained_symptom_to_disease_model.pth', map_location=torch.device('cpu')))
model.eval()  # Set to evaluation mode

# Disease Advice (unchanged)
disease_advice = {
...
with gr.Blocks(css="""#col_container { margin-left: auto; margin-right: auto;} #chatbot {height: 520px; overflow: auto;}""") as demo:
    ...
    def respond(message, chat_history):
        if message.lower() in greetings:
            ...
            bot_message = f'Based on your symptoms, I believe you may have {test_pred}. I would advise: {disease_advice[test_pred]} Remember, this is not a substitute for professional medical advice—consult a doctor.'
